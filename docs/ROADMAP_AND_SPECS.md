...
| ドキュメント | 目的 | 参照タイミング |
| --- | --- | --- |
| [workflow-cookbook/HUB.codex.md](../workflow-cookbook/HUB.codex.md) | Day8 の変更要求を内部観測イベントへマッピングし、必要な一次資料を抽出する | 新規課題が発生した直後に参照し、影響範囲を整理するとき |
...
- [workflow-cookbook/HUB.codex.md](../workflow-cookbook/HUB.codex.md): 変更要求を Day8 内の観測イベントへ対応付け、初動で必要な資料一覧を洗い出すときに使う。Day8 の Task Seed 運用と命名規約（[docs/TASKS.md](TASKS.md) / [docs/seeds/README.md](seeds/README.md)）および Birdseye 更新手順（[docs/birdseye/README.md](birdseye/README.md)）を前提にノードへ紐付ける。
...
| --- | --- | --- | --- | --- |
| 要求 | ユーザー課題の把握とスコープ設定 | [docs/day8/spec/01_requirements.md](day8/spec/01_requirements.md) | [workflow-cookbook/BLUEPRINT.md](../workflow-cookbook/BLUEPRINT.md) | Day8 のフロー/SLO/フェイルセーフを BLUEPRINT 章立てと突き合わせ、固定事項/ユースケース/FR/NFR/データモデル/受入基準/マイルストーンへ翻訳。Appendix G/J/K/M1 と Release Checklist を同時確認 |
...
| ガバナンス | 優先度・承認ルール | [governance/policy.yaml](../governance/policy.yaml) | [workflow-cookbook/governance/policy.yaml](../workflow-cookbook/governance/policy.yaml) / [workflow-cookbook/governance/prioritization.yaml](../workflow-cookbook/governance/prioritization.yaml) | Day8 ガバナンスの意思決定履歴を記録し、優先度判定を同期。[docs/UPSTREAM.md](UPSTREAM.md) / [docs/UPSTREAM_WEEKLY_LOG.md](UPSTREAM_WEEKLY_LOG.md) で承認ログと週次レビューを補完 |
...
5. **Birdseye / ADR の整合を取る** — [docs/birdseye/index.json](birdseye/index.json)・[docs/day8/design/03_architecture.md](day8/design/03_architecture.md) を更新し、可視化ビューと設計判断をルート [CHANGELOG.md](../CHANGELOG.md) と [workflow-cookbook/CHANGELOG.md](../workflow-cookbook/CHANGELOG.md) の双方で同期する。
...
- [Day8 Guardrails サマリ](../workflow-cookbook/GUARDRAILS.md) — 制約とレビュー基準を Day8 の実装方針へ即時反映。Birdseye フロー（[docs/birdseye/README.md](birdseye/README.md)）と Task Seed の命名規約（[docs/seeds/README.md](seeds/README.md)）を併用してチェックリストへ反映する。
...
- [Day8 Release Checklist](Release_Checklist.md) — ops/quality 文書と同期したリリース判定の必須項目。Day8 のリリース運用付録（[docs/addenda/H_Deploy_Guide.md](addenda/H_Deploy_Guide.md) / [docs/addenda/M_Versioning_Release.md](addenda/M_Versioning_Release.md)）と合わせて使用する。
...
- [ガバナンス方針](../workflow-cookbook/governance/policy.yaml) — 優先度・承認ルールの一次情報。参考: [workflow-cookbook/governance/prioritization.yaml](../workflow-cookbook/governance/prioritization.yaml)
- [Versioning & Release Operations（Appendix M）](addenda/M_Versioning_Release.md) — バージョニングとリリース運用の補足資料。
- [Day8 コントリビューションガイド](day8/guides/07_contributing.md) — 実装/レビュー/テストの標準フロー。
- [Day8 マイルストーン WBS](day8_wbs.csv) — 計画策定とスプリントレビューでマイルストーン進捗を照合し、ロードマップとの差分を抽出する際に利用。
- [Birdseye フォールバック手順](BIRDSEYE.md) — 可視化更新時の確認ポイント。参考: [docs/birdseye/README.md](birdseye/README.md)
